// RaK 1.2 (1.82) – Supabase bridge a online synchronizace.
(function () {
  ...
  const SUPABASE_POLICY_AUDIT_SNAPSHOT_VERSION = '1.2 (1.82)';
  ...
  function bindRealtimeSubscriptions() {
    ...
    try {
      ...
      const channel = client.channel('rak-public-live-v1-2-1-82');
  ...
  function buildAnnouncementInsertAttempts(payload, nowIso) {
    const rowFull = cleanAnnouncementRow({
      ...
      updated_at: nowIso,
      app_version: String(window.APP_VERSION || '1.2 (1.82)'),
      priority: 0
    });
    return [
  ...
  async function saveDashboardAnnouncementViaRpc(client, safe, nowIso) {
    try {
      const res = await runSupabaseOperation('announcements.rpc-save', () => client.rpc('rak_save_dashboard_announcement', {
        ...
        p_app_version: String(window.APP_VERSION || '1.2 (1.82)'),
        p_priority: 0
  ...
  function getDashboardAnnouncementOnlineStatus() {
    ...
    return Object.assign({}, status, {
      ...
      table: 'announcements',
      realtimeChannel: 'rak-public-live-v1-2-1-82',
