...
// v.1.6 (1007): Administrace / Reporty chyb jsou oddělené v admin-reports.js.

// v.1.6 (1007): Administrace / Přehled připojení, servis a oznámení jsou oddělené v admin-service-usage.js.
...
function adminShowRotationSelectedRemove(input) {
  try {
    ...
    // v.1.6 (1007): horní sticky tlačítko už při kliknutí do jména nevytahujeme.
...
function scheduleRotaceNamesDockMetrics(reason) {
  ...
  try {
    const root = document.documentElement;
    if (root && root.dataset) root.dataset.rakRotaceNamesDockMode = 'static-css-bottom-v1007';
    if (typeof requestAnimationFrame === 'function') {
      requestAnimationFrame(() => { try { updateRotaceNamesDockMetrics(reason || 'static-css-bottom-v1007'); } catch (err) {} });
    }
    return updateRotaceNamesDockMetrics(reason || 'static-css-bottom-v1007');
  } catch (err) {
    try { return updateRotaceNamesDockMetrics(reason || 'static-css-bottom-v1007-fallback'); } catch (_) { return null; }
  }
}
