...
// v.1.6 (1008): Administrace / Rozpisy a Nastavení strojů jsou oddělené v admin-rotation.js.




// v.1.6 (1008): Administrace / Reporty chyb jsou oddělené v admin-reports.js.

// v.1.6 (1008): Administrace / Přehled připojení, servis a oznámení jsou oddělené v admin-service-usage.js.
...
// v.1.6 (1008): Plovoucí odebrání a údržba editoru rozpisů jsou oddělené v admin-rotation.js.
...
function scheduleRotaceNamesDockMetrics(reason) {
  ...
  try {
    const root = document.documentElement;
    if (root && root.dataset) root.dataset.rakRotaceNamesDockMode = 'static-css-bottom-v1008';
    if (typeof requestAnimationFrame === 'function') {
      requestAnimationFrame(() => { try { updateRotaceNamesDockMetrics(reason || 'static-css-bottom-v1008'); } catch (err) {} });
    }
    return updateRotaceNamesDockMetrics(reason || 'static-css-bottom-v1008');
  } catch (err) {
    try { return updateRotaceNamesDockMetrics(reason || 'static-css-bottom-v1008-fallback'); } catch (_) { return null; }
  }
}
