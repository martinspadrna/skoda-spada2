-- RaK v.1.5 (837) – Supabase keepalive přes RPC místo přímého RLS upsertu.
...
comment on table public.app_keepalive is 'RaK v837: lightweight app startup heartbeat storage. Client writes through RPC rak_app_keepalive only; no direct anon/auth table access. Separate from game data and does not touch game_invites/game_sessions policies.';

comment on function public.rak_app_keepalive(text, text, text, jsonb) is 'RaK v837: safe app keepalive RPC. Validates/truncates client fields and writes only to app_keepalive. Does not touch game_invites/game_sessions.';
