...
(function () {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .rakShiftIndex{font-weight:800;transition:border-color .15s ease,background .15s ease,box-shadow .15s ease}.rakShiftIndex--blue{color:#86c8ff;border-color:rgba(74,166,255,.58);background:rgba(35,111,190,.18);box-shadow:inset 0 0 0 1px rgba(74,166,255,.08)}.rakShiftIndex--green{color:#8ee6a4;border-color:rgba(62,190,96,.58);background:rgba(39,142,69,.18);box-shadow:inset 0 0 0 1px rgba(62,190,96,.08)}.rakShiftIndex--orange{color:#ffc178;border-color:rgba(255,151,47,.62);background:rgba(195,101,20,.18);box-shadow:inset 0 0 0 1px rgba(255,151,47,.08)}
  ...
  function syncIndexColor(select) {
    ...
    select.dataset.indexColor = INDEX_COLORS[select.value] || 'neutral';
  }
  ...
  function addIndex(root, id) { const block = root.querySelector('[data-section-block="' + id + '"] .rakShiftRows'); const section = SECTIONS.find((s) => s.id === id); if (!block || !section) return; block.insertAdjacentHTML('beforeend', sectionRow(section, { index: section.defaultIndex })); renderPreview(root); }
