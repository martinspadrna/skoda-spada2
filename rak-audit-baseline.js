// v.1.5 (922) – release/architecture readiness bere i release ops checklist jako read-only signál.
...
function getRakReleaseReadinessHealth() {
  ...
  return {
    ok: issues.length === 0,
    mode: 'audit-baseline-split-release-readiness-v922',
    checkedAt,
...
function getRakArchitectureBaselineHealth() {
  ...
  return {
    ok: issues.length === 0,
    mode: 'audit-baseline-split-architecture-boot-audit-v922',
    checkedAt,
