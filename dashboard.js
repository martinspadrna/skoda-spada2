...
// v.1.5 (955) – Dashboard oznámení je globální online-first; localStorage slouží jen jako rychlá cache/fallback.
...
function readRakLocalDashboardAnnouncement() {
  try {
    // v.1.5 (955): čteme přímo z localStorage, aby se po restartu / návratu z bfcache
...
function getRakDashboardAnnouncementCandidates() {
  const items = [];
  // v.1.5 (955): globální oznámení má přednost. Lokální kopie je jen cache/fallback,
...
// v.1.5 (955) – read-only kontrola centrování horního směnového panelu na Dashboardu včetně řádku kdo chybí.
