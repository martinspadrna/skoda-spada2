...
-- Trigger helpery mají pevný search_path, aby jejich chování nemohl ovlivnit session search_path.
...
-- Hradník je ve stejném Supabase projektu; tato změna je pouze bezpečné zpevnění trigger helperu.
alter function public.hradnik_touch_updated_at() set search_path = '';

-- Chybějící indexy nad FK zjištěné Supabase Advisorem.
create index if not exists rak_admin_audit_log_user_id_idx
  on public.rak_admin_audit_log (user_id);
create index if not exists rak_admin_devices_revoked_by_idx
  on public.rak_admin_devices (revoked_by);
create index if not exists rak_admin_profiles_created_by_idx
  on public.rak_admin_profiles (created_by);
create index if not exists rak_admin_settings_backups_created_by_idx
  on public.rak_admin_settings_backups (created_by);
create index if not exists rak_admin_settings_backups_restored_backup_id_idx
  on public.rak_admin_settings_backups (restored_backup_id);
create index if not exists rak_rotation_backups_v2_created_by_idx
  on public.rak_rotation_backups_v2 (created_by);

-- SECURITY DEFINER admin/owner RPC jsou určené pouze přihlášeným správcům.
-- rak_admin_account_requires_auth je jediná výjimka: login ho potřebuje ještě před Supabase Auth
-- a funkce vrací pouze boolean, zda daný účet vyžaduje admin přihlášení.
do $$
declare
  fn record;
begin
...
-- Hry jsou z RaK odstraněné. Staré herní SECURITY DEFINER RPC proto už nesmí být
-- veřejnou zapisovací plochou. Datové tabulky nemažeme, aby zůstala historie a rollback.
do $$
declare
  fn record;
begin
...
-- Staré herní tabulky už RaK nepotřebuje veřejně číst. Samotná data zůstávají zachována.
...
-- machine_settings: místo dvou permissive SELECT policies pro authenticated držíme jednu.
