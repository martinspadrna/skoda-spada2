// RaK 1.2 (1.153) – arcade hry a denní challenge.
(() => {
  ...
  // RaK 1.2 (1.153): online score časových her musí projít stávajícím Supabase RPC limitem
  ...
  function getArcadeProfileDisplay(account, gameId) {
    ...
    if (id === 'ttt') return `${Number(st.plays || 0) || 0}×`;
    if (meta.mode === 'low') {
      const safeTime = gamesSanitizeLowBestTime(id, st.bestTimeMs || st.leaderboardValue || 0);
      return safeTime ? fmtGameValue(id, safeTime) : '—';
    }
    return `${Number(st.bestScore || st.leaderboardValue || 0) || 0}`;
  }
  ...
  function getRakGamesAchievementRewardHealth() {
    ...
    return {
      version: window.APP_VERSION || '1.2 (1.153)',
...
`;
  ...
  function getRakGamesTopScoreDomHardeningHealth() {
    ...
    return {
      ...
      version: String(window.APP_VERSION || '1.2 (1.153)'),
  ...
  function getRakGamesTopScoreSecondsHealth() {
    const reactionProbe = fmtGameValue('reaction', 184);
    const sudokuProbe = fmtGameValue('sudoku', 125000);
    const ok = reactionProbe.indexOf('ms') >= 0 && sudokuProbe.indexOf('min') >= 0 && sudokuProbe.indexOf('s') >= 0;
    return {
      ok,
      ...
      reactionProbe,
      sudokuProbe,
      note: 'Časové Top score používá sekundy, nad minutu min + s; Reaction Test zůstává jediný v milisekundách.'
    };
  }
  ...
  function fmtGameValue(gameId, ms) { return key(gameId) === 'reaction' || key(gameId) === 'daily_reaction' ? fmtReactionMs(ms) : fmtSeconds(ms); }
  window.RaKGamesFormatTimeValue = fmtGameValue;
  function gamesIsMemoryLike(id) { const gid = key(id); return gid === 'memory' || /^memory_\d+x\d+$/.test(gid) || gid === 'daily_memory'; }
  function gamesMemoryMinValidMs(gameId) {
    const gid = key(gameId);
    if (gid === 'memory_8x8') return 60000;
    if (gid === 'memory_6x6') return 30000;
    if (gid === 'memory' || gid === 'memory_4x4' || gid === 'daily_memory') return 12000;
    return 0;
  }
  function gamesSanitizeLowBestTime(gameId, ms) {
    const n = Number(ms) || 0;
    if (!Number.isFinite(n) || n <= 0) return 0;
    // RaK 1.2 (1.153): starý 5s fallback Pexesa není platný výherní čas.
    // Pexeso musí používat celkový čas od startu hry; nereálné krátké lokální/cache záznamy se schovají
    // a při remote syncu je může nahradit skutečný online čas.
    const memoryMinMs = gamesMemoryMinValidMs(gameId);
    if (memoryMinMs > 0 && n < memoryMinMs) return 0;
  ...
  function lowBetterUsesMs(id) {
    const gid = key(id);
    return gid === 'reaction' || gid === 'daily_reaction';
  }
  function lowBetterMetricFromTime(id, value) {
    const n = Number(value) || 0;
    if (!Number.isFinite(n) || n <= 0) return 0;
    if (lowBetterUsesMs(id)) return Math.max(1, Math.round(n));
    return Math.max(1, Math.round(n / 1000));
  }
  function encodePoints(id, value) {
    const v = Number(value) || 0;
    if (!isLowBetter(id)) return Math.max(0, Math.round(v));
    const metric = lowBetterMetricFromTime(id, v);
    if (!metric) return 0;
    // Bezpečný zápis do Supabase: vyšší score = lepší čas, ale zůstává do 5000.
    return Math.max(1, Math.min(LOW_TIME_SAFE_POINT_MAX, LOW_TIME_SAFE_POINT_SCALE - Math.min(LOW_TIME_SAFE_POINT_MAX, metric)));
  }
  function decodePoints(id, value) {
    ...
    if (isLowBetter(id)) {
      ...
      if (!rounded) return 0;
      // Zpětná kompatibilita s krátce používaným zápisem 1e9 - čas_ms.
      if (rounded > 86400000 && rounded < POINT_SCALE) return Math.max(0, POINT_SCALE - rounded);
      // Nový bezpečný zápis do 5000: Reaction v ms, ostatní časové hry v sekundách.
      if (rounded > 0 && rounded < LOW_TIME_SAFE_POINT_SCALE) {
        const metric = Math.max(1, LOW_TIME_SAFE_POINT_SCALE - rounded);
        return lowBetterUsesMs(id) ? metric : metric * 1000;
      }
      // Starší řádky s raw hodnotou necháváme čitelné.
  ...
  function renderSudoku(body) {
    ...
    const completeCheck = () => {
      ...
      if (valid && !state.finished) {
        ...
        const sudokuResultPatch = { completed: true, plays: 1, wins: 1, bestTimeMs: time, timeMs: time, elapsedMs: time, difficulty: state.selected, lastResult: `${pick.label || state.selected} · ${fmtTime(time)}` };
        ...
        window.__rakLastSudokuCompletion = { boardId: sudokuBoardId, difficulty: state.selected, timeMs: time, safeScore: encodePoints(sudokuBoardId, time), recordedAt: Date.now(), seed: state.sudokuSeed || '', version: '1.2 (1.153)' };
  ...
  // RaK 1.2 (1.153): Pexeso 8×8 musí mít tvrdý runtime fit, protože starší globální CSS
  ...
  function renderMemory(body) {
    ...
    body.innerHTML = `
      <div class="arcadeStage memoryStage memoryStage-${size}" data-memory-size="${size}">
        ...
        <div class="arcadeGridList grid-${size} arcadePanel arcadeMemoryBoard arcadeMemoryBoardLarge" id="memoryGrid" data-memory-grid="${size}">${cells}</div>
    ...
    const finishIfDone = () => {
      if (state.matched.size >= state.deck.length && !state.over) {
        ...
        gamesRecordStat('memory', { completed: true, wins: 1, plays: 1, bestTimeMs: totalTimeMs, timeMs: totalTimeMs, elapsedMs: totalTimeMs, bestScore: encodePoints('memory', totalTimeMs), bestMoves: state.moves, bestSize: size, difficulty: `${size}x${size}`, lastResult: `${size}×${size} · ${fmtTime(totalTimeMs)} · ${state.moves} tahů` });
        gamesRecordStat(memoryBoardId, { completed: true, wins: 1, plays: 1, bestTimeMs: totalTimeMs, timeMs: totalTimeMs, elapsedMs: totalTimeMs, bestScore: encodePoints(memoryBoardId, totalTimeMs), bestMoves: state.moves, bestSize: size, difficulty: `${size}x${size}`, lastResult: `${size}×${size} · ${fmtTime(totalTimeMs)} · ${state.moves} tahů` });
        const controls = body.querySelector('.arcadeControls');
  ...
  // RaK 1.2 (1.153): Pexeso 8×8 musí na mobilu držet čtvercové karty a vejít se pod HUD.
  if (typeof window !== 'undefined') {
    window.__rakMemory8x8SquareFitGuard = {
      version: String(window.APP_VERSION || '1.2 (1.153)'),
      mode: 'memory-8x8-square-fit-v1153-guard',
  ...
  // RaK 1.2 (1.153): Lodě nesmí překreslovat celou 10×10 tabuli při každém pollu.
  ...
  function renderShips(body) {
    ...
    const local = getState('ships', () => ({ code: '', state: null, loadedRevision: 0, busy: false, refreshing: false, poll: 0, pollDelay: 0, savedFinishedCode: '', view: '', lastTurn: '', lastStatus: '', placing: { manual: false, selected: 'carrier', horizontal: true } }));
    local.refreshing = false;
    local.pollDelay = Number(local.pollDelay || 0) || 0;
    ...
    const shipsDesiredPollDelay = (st, role) => {
      const state = st ? shipsNormalizeState(st, local.code) : null;
      if (!state || state.status === 'finished') return 0;
      if (state.status === 'active') return SHIPS_POLL_ACTIVE_MS;
      if (role && !state[readyKey(role)]) return SHIPS_POLL_IDLE_MS;
      return SHIPS_POLL_IDLE_MS;
    };
    const ensureShipsPoll = (st) => {
      if (!local.code) return;
      const role = st ? shipsRoleForState(st) : '';
      const delay = shipsDesiredPollDelay(st || local.state, role);
      if (!delay) {
        if (local.poll) clearInterval(local.poll);
        local.poll = 0;
        local.pollDelay = 0;
        return;
      }
      if (local.poll && local.pollDelay === delay) return;
      if (local.poll) clearInterval(local.poll);
      local.pollDelay = delay;
      local.poll = rakGameSetInterval(() => refreshRemote(true), delay);
    };
    const maybeRenderRemoteState = (remote, soft) => {
      const oldState = local.state || {};
      const oldRev = Number(oldState.revision || 0) || 0;
      const nextRev = Number(remote && remote.revision || 0) || 0;
      const oldStatus = String(oldState.status || '');
      const newStatus = String(remote && remote.status || '');
      const opponentChanged = String(oldState.playerOAccountNumber || '') !== String(remote.playerOAccountNumber || '') || (!!oldState.o !== !!remote.o);
      const statusChanged = oldStatus !== newStatus || String(oldState.turn || '') !== String(remote.turn || '');
      const becameActive = oldStatus !== 'active' && newStatus === 'active' && !!remote.xReady && !!remote.oReady && !!remote.x && !!remote.o;
      const revisionChanged = nextRev > oldRev || (!oldRev && !!nextRev);
      if (!soft || opponentChanged || statusChanged || becameActive || revisionChanged) {
        local.state = remote;
    ...
    const refreshRemote = async (soft) => {
      ...
      try {
        ...
        if (!remote) return;
        maybeRenderRemoteState(remote, soft);
        ensureShipsPoll(remote);
      } catch (err) {
        try { console.warn('ships refresh failed', err); } catch (logErr) {}
      } finally {
        local.refreshing = false;
      }
    };
    ...
    const renderGame = (fromPoll) => {
      ...
      body.querySelectorAll('[data-ships-shot]').forEach((btn) => {
        btn.addEventListener('click', async () => {
          ...
          local.state = stNow;
          renderGame();
          void shipsSaveState(stNow).then(() => {
            maybeRecordFinished();
            ensureShipsPoll(stNow);
            return refreshRemote(true);
          }).catch((err) => {
            try { console.warn('ships save shot failed', err); } catch (logErr) {}
          });
        });
      });
      if (!fromPoll && local.poll) { clearInterval(local.poll); local.poll = 0; local.pollDelay = 0; }
      ensureShipsPoll(st);
    };
    addCleanup(() => { if (local.poll) clearInterval(local.poll); local.poll = 0; local.pollDelay = 0; local.refreshing = false; });
  ...
  // RaK 1.2 (1.153): smoke guard pro plynulost Lodí – žádný render při každém pollu beze změny.
  if (typeof window !== 'undefined') {
    window.__rakShipsSmoothPerformanceGuard = {
      ...
      avoidsRenderWithoutRevisionChange: true,
      preventsOverlappingRefresh: true,
      immediateShotRender: true
    };
  }
  ...
  // RaK 1.2 (1.153): Pexeso nesmí zobrazovat starý 5s fallback jako platný čas.
  window.RAK_MEMORY_TIME_SANITIZE_V1152 = {
    version: String(window.APP_VERSION || '1.2 (1.153)'),
    mode: 'memory-total-time-no-5s-v1153-guard',
    memory4x4FiveSecondsIsInvalid: gamesSanitizeLowBestTime('memory_4x4', 5000) === 0,
    memory6x6FiveSecondsIsInvalid: gamesSanitizeLowBestTime('memory_6x6', 5000) === 0,
    memory8x8FiveSecondsIsInvalid: gamesSanitizeLowBestTime('memory_8x8', 5000) === 0,
    memory82SecondsIsValid: gamesSanitizeLowBestTime('memory_6x6', 82000) === 82000
  };
  ...
  function getRakSudokuCompletionSaveHealth() {
    const sampleTime = 125000;
    const basePoints = encodePoints('sudoku', sampleTime);
    const variantPoints = encodePoints('sudoku_easy', sampleTime);
    const decodedBase = decodePoints('sudoku', basePoints);
    const reactionPoints = encodePoints('reaction', 184);
    return {
      ...
      sampleSafeScore: basePoints,
      sampleVariantSafeScore: variantPoints,
      reactionSafeScore: reactionPoints,
      sampleTimeText: fmtGameValue('sudoku', sampleTime),
      reactionTimeText: fmtGameValue('reaction', 184),
      note: 'Sudoku po vyřešení zapisuje společné Sudoku i obtížnost. Online score časových her je bezpečně do 5000, XP zůstává vyrovnané a UI ho dekóduje zpět na čas.'
    };
  }
