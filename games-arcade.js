(() => {
  ...
  // v.1.1 (729): export/import opravený; Pampuch zůstává ze 727 zpomalený a směnové achievementy jen pro D.
  ...
  function gamesBuildCompletionContext(timestamp) {
    ...
    // v.1.1 (729): herní achievementy s podmínkou „ve směně“ se počítají jen tehdy,
...
`;
  ...
  function renderLaunchTiles() {
    ...
    if (!grid) return;
    const launchSig = CORE_GAMES.join('|') + '::' + EXTRA_GAMES.join('|') + '::v729';
  ...
  function runArcadeGamesFullAudit() {
    ...
    return {
      ...
      touchGuard: true,
      notes: [
        'Build 729 opravuje dvoukrokový import Excelu s výběrem načteného měsíce a funkční export ZIP buildu.',
