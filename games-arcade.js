// RaK 1.2 (1.149) – arcade hry a denní challenge.
(() => {
  ...
  // RaK 1.2 (1.149): online score časových her musí projít stávajícím Supabase RPC limitem
...
`;
  ...
  function getRakGamesTopScoreDomHardeningHealth() {
    ...
    return {
      ...
      version: String(window.APP_VERSION || '1.2 (1.149)'),
  ...
  function renderSudoku(body) {
    ...
    const completeCheck = () => {
      ...
      if (valid && !state.finished) {
        ...
        window.__rakLastSudokuCompletion = { boardId: sudokuBoardId, difficulty: state.selected, timeMs: time, safeScore: encodePoints(sudokuBoardId, time), recordedAt: Date.now(), seed: state.sudokuSeed || '', version: '1.2 (1.149)' };
  ...
  // RaK 1.2 (1.149): Lodě nesmí překreslovat celou 10×10 tabuli při každém pollu.
  ...
  // RaK 1.2 (1.149): smoke guard pro plynulost Lodí – žádný render při každém pollu beze změny.
