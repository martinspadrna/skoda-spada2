// RaK 1.2 (1.152) – theme, pozadí a profilové UI nastavení.
...
const RAK_THEME_DEFS = [
  ...
  },
  {
    "id": "amoled-emerald",
    "label": "AMOLED zelený",
    "subtitle": "Černý základ se zelenými akcenty pro AMOLED displeje",
    "color": "#34D399",
    "unlockText": "12 her nebo 3 achievementy",
    "minPlays": 12,
    "minAchievements": 3,
    "vars": {
      "--bg": "#000000",
      "--panel": "rgba(5,12,10,.92)",
      "--panel2": "rgba(8,18,15,.84)",
      "--green": "#34D399",
      "--green2": "#A7F3D0",
      "--text": "#ECFFF8",
      "--muted": "#9AB8AF",
      "--soft": "#F5FFFB",
      "--rakThemeGlow": "rgba(52,211,153,.30)",
      "--rakThemeBorder": "rgba(52,211,153,.25)",
      "--rakThemeAccentStrong": "#34D399",
      "--rakThemeAccentSoft": "#A7F3D0"
    }
  }
];
...
const RAK_BACKGROUND_DEFS = [
  ...
  },
  {
    "id": "amoled-grid",
    "label": "AMOLED mřížka",
    "subtitle": "Skoro černé pozadí s jemnou modrou mřížkou pro OLED displeje",
    "color": "#38BDF8",
  ...
  },
  {
    "id": "amoled-pulse",
    "label": "AMOLED pulse",
    "subtitle": "Černý základ s jemnými zeleno-tyrkysovými šikmými linkami",
    "color": "#34D399",
    "swatch": "repeating-linear-gradient(135deg, rgba(52,211,153,.24) 0 2px, transparent 2px 18px), linear-gradient(180deg, #000000 0%, #040806 100%)",
    "unlockText": "14 her nebo 4 achievementy",
    "minPlays": 14,
    "minAchievements": 4,
    "vars": {
      "--rakBgBase": "#000000",
      "--rakAppBackground": "repeating-linear-gradient(135deg, rgba(52,211,153,.095) 0 1px, transparent 1px 24px), radial-gradient(circle at 18% 18%, rgba(45,212,191,.12), transparent 28%), linear-gradient(180deg, #000000 0%, #040806 58%, #0a1210 100%)",
      "--rakAppBackgroundOverlay": "linear-gradient(90deg, rgba(0,0,0,.72), transparent 24%, rgba(52,211,153,.030) 50%, transparent 76%, rgba(0,0,0,.72))",
      "--rakAppBackgroundLite": "linear-gradient(180deg, #000000 0%, #040806 62%, #0a1210 100%)",
      "--rakBgAccent": "rgba(52,211,153,.24)",
      "--green": "#34D399",
      "--green2": "#A7F3D0"
    }
  }
...
// RaK 1.2 (1.152) – patternové varianty pro pracovní světlá pozadí.
...
const RAK_PATTERN_BACKGROUND_META_V1132 = Object.freeze({
  'skoda-green': Object.freeze({ label: 'Škoda linky', subtitle: 'Světlý smetanový podklad se zelenými šikmými linkami' }),
  'light-green': Object.freeze({ label: 'Mint mřížka', subtitle: 'Jemný světlý základ s viditelnější mint mřížkou' }),
  'deep-aurora': Object.freeze({ label: 'Modrý blueprint', subtitle: 'Světlý modrý plánový vzor s jemnou mřížkou' }),
  'ember': Object.freeze({ label: 'Pískové šipky', subtitle: 'Teplý pískový podklad s úhlovými linkami' }),
  'neon-lagoon': Object.freeze({ label: 'Lagoon vlnky', subtitle: 'Světlý aqua základ s jemně vlnitými linkami' }),
  'electric-lime': Object.freeze({ label: 'Limetkové řezy', subtitle: 'Světlý citronový podklad s diagonálními řezy' }),
  'skoda-electric': Object.freeze({ label: 'Zelený okruh', subtitle: 'O něco tmavší pastelový podklad se zeleným linkovým okruhem' }),
  'candy-glass': Object.freeze({ label: 'Růžové střípky', subtitle: 'Tlumenější růžový podklad se šikmými střepy' }),
  'aurora-punch': Object.freeze({ label: 'Fialové zlomy', subtitle: 'Tlumená fialová se zalomenými diagonálami' }),
  'violet-storm': Object.freeze({ label: 'Levandulový cikcak', subtitle: 'Tmavší levandulový podklad s cikcak vzorem' }),
  'sunset-plasma': Object.freeze({ label: 'Meruňkové linky', subtitle: 'Meruňkový základ s výraznějšími šikmými tahy' }),
  'polar-mint': Object.freeze({ label: 'Ledové čáry', subtitle: 'Chladnější světle modrý podklad s rovnými čarami' }),
  'blue-orbit': Object.freeze({ label: 'Modré orbity', subtitle: 'Tlumený modrý podklad s kruhovými stopami' })
});
...
function applyPatternBackgroundsV1132() {
  RAK_BACKGROUND_DEFS.forEach((bg) => {
    const id = String(bg && bg.id || '');
    const meta = RAK_PATTERN_BACKGROUND_META_V1132[id];
    const variant = RAK_PATTERN_BACKGROUND_VARIANTS_V1132[id];
    if (!meta || !variant) return;
    bg.label = meta.label;
    bg.subtitle = meta.subtitle;
    bg.color = variant.green;
    bg.swatch = buildRakPatternSwatchV1132(variant);
    bg.vars = Object.assign({}, bg.vars || {}, {
      '--rakBgBase': variant.bottom,
      '--rakAppBackground': buildRakPatternBackgroundV1132(variant),
      '--rakAppBackgroundOverlay': 'linear-gradient(90deg, rgba(255,255,255,.28), transparent 24%, rgba(0,0,0,.03) 50%, transparent 76%, rgba(255,255,255,.24))',
      '--rakAppBackgroundLite': 'linear-gradient(180deg, ' + variant.top + ' 0%, ' + variant.mid + ' 58%, ' + variant.bottom + ' 100%)',
      '--rakBgAccent': variant.high,
      '--green': variant.green,
      '--green2': variant.green2
    });
  });
}
applyPatternBackgroundsV1132();
function applyLightPatternBackgroundsV1131() { return applyPatternBackgroundsV1132(); }
...
const RAK_BACKGROUND_UNLOCKS_V927 = {
  ...
  'amoled-grid': { unlockText: 'Vždy dostupné', minPlays: 0, minAchievements: 0 },
  'amoled-pulse': { unlockText: '14 her nebo 4 achievementy', minPlays: 14, minAchievements: 4 },
...
// RaK 1.2 (1.152) – appearance reward contract guard.
...
const RAK_APPEARANCE_REWARD_CONTRACT_V181 = Object.freeze({
  ...
  basicBackgroundIds: Object.freeze(['ios-mesh', 'light-zigzag', 'amoled-grid']),
  rules: Object.freeze([
  ...
  themeFamilies: Object.freeze({
    ...
    neutralPremium: Object.freeze(['graphite', 'royal-gold', 'amoled-legend', 'amoled-emerald'])
  }),
  backgroundFamilies: Object.freeze({
    ...
    amoled: Object.freeze(['amoled-grid', 'amoled-pulse'])
  }),
...
// RaK 1.2 (1.152) – appearance readability contract guard.
...
function applyProfileUiPreferencesForActiveAccount(options = {}) {
  ...
  let changed = false;
  // RaK 1.2 (1.152): při aktualizaci nesmí prázdné profilové uiSettings shodit uživatelské pozadí zpět na základ.
  ...
  // RaK 1.2 (1.152): uložený vzhled profilu nepřepisujeme na default jen proto,
  // že nové zařízení ještě nestihlo stáhnout online statistiky a dopočítat rank.
  const savedThemeUnlocked = isAppearanceRewardUnlocked(savedTheme, rewardMetrics, 'default');
  const savedBgUnlocked = isAppearanceRewardUnlocked(savedBg, rewardMetrics, 'ios-mesh');
  const themeToApply = savedThemeUnlocked ? (ui.themeId || defaultTheme) : defaultTheme;
  const bgToApply = savedBgUnlocked ? (ui.backgroundId || defaultBg) : defaultBg;
  if (changed || !ui.updatedAt) {
    ui.updatedAt = Date.now();
    account.updatedAt = Math.max(Number(account.updatedAt || 0) || 0, ui.updatedAt);
    profile.profileVersion = GAMES_PROFILE_RESET_VERSION;
...
const RAK_APPEARANCE_LOCKED_PREFERENCE_PRESERVE_CONTRACT_V1144 = Object.freeze({
  version: '1.2 (1.152)',
  scope: 'profile-appearance-rank-sync',
  issue: 'nové zařízení nesmí smazat uložené theme/pozadí dřív, než se stáhne rank profilu',
  behavior: 'zamčený uložený skin se dočasně vizuálně nahradí defaultem, ale hodnota v account.uiSettings zůstane zachovaná',
  recovery: 'po stažení statistik aktivního účtu se uložený skin znovu aplikuje, pokud je podle ranku/achievementů odemčený'
});
window.RAK_APPEARANCE_LOCKED_PREFERENCE_PRESERVE_CONTRACT_V1144 = RAK_APPEARANCE_LOCKED_PREFERENCE_PRESERVE_CONTRACT_V1144;
