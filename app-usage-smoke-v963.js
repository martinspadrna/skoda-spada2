...
const appPwaConnectivityJs = read('app-pwa-connectivity.js');
...
function assertReleaseMetadataContractV199() {
  ...
  assertIncludes(appMenuJs, 'function getRakCurrentAppVersion()', 'O aplikaci musí brát verzi z aktuálního buildu, ne ze starého uloženého stavu');
  assertIncludes(appMenuJs, 'const versionText = getRakCurrentAppVersion();', 'O aplikaci musí zobrazit aktuální produkční verzi');
  assertIncludes(appPwaConnectivityJs, 'let waitingForActivation = false;', 'PWA aktualizace musí rozlišit čekající service worker');
  assertIncludes(appPwaConnectivityJs, "scheduleUpdateReload('manual-fallback')", 'PWA aktualizace musí mít záložní reload při opožděné aktivaci');
  assertIncludes(appPwaConnectivityJs, "registerListener(navigator.serviceWorker, 'controllerchange'", 'PWA aktualizace musí čekat na převzetí řízení novým service workerem');
  assertIncludes(serviceWorkerJs, "approvedUpdateClientId = String(event.source && event.source.id || '');", 'Aktivovaný service worker si musí pamatovat klienta, který schválil aktualizaci');
  assertIncludes(serviceWorkerJs, 'client.navigate(client.url)', 'Po schválené aktualizaci musí service worker otevřít klienta už pod novou cache');
  assertIncludes(ui, "range: 'RaK 1.5'", 'O aplikaci musí obsahovat krátký uživatelský souhrn vydání RaK 1.5');
...
function assertRotationGeneratorRulesContractV1116() {
  ...
  assertIncludes(ui, 'relieveKindHeavyWithAnyOpposite', 'Dorovnání MFKF/MSKC nesmí čekat jen na jiného člověka přetíženého opačným typem práce');
...
function assertAdminAccountLoginContractV1141() {
  ...
  assertIncludes(adminUnlockJs, 'Muzou spravovat pracovni casti administrace a zmenit vlastni heslo, ale nemuzou menit dalsi adminy ani heslo hlavniho admina.', 'Nizsi admini smi menit jen vlastni heslo, ne heslo ownera ani dalsich adminu');
  ...
  assertIncludes(adminUnlockJs, 'function buildAdminOwnPasswordHtml', 'Nizsi admin musi mit vlastni zabezpecenou zmenu hesla');
  assertIncludes(adminUnlockJs, 'function rakAdminChangeOwnPassword', 'Zmena hesla nizsiho admina musi mit vlastni obsluhu');
  assertIncludes(adminUnlockJs, "action: 'change-own-password'", 'Nizsi admin musi menit heslo jen pres serverovou akci pro vlastni ucet');
  assertIncludes(adminUnlockJs, 'adminAccountsMaskedPassword', 'Hesla v prehledu spravcu musi byt maskovana');
  assertIncludes(adminUnlockJs, '>****</span>', 'Prehled spravcu nesmi zobrazit skutecna hesla');
  assertIncludes(ui, "adminAction === 'change-own-admin-password'", 'Menu musi obslouzit zmenu vlastniho hesla nizsiho admina');
  assertIncludes(ui, "owner ? 'Hlavní admin' : 'Nižší admin'", 'Admin uvod musi nizsiho admina pojmenovat srozumitelne');
  assertIncludes(ui, 'ale ne hesla dalších správců ani hlavního admina', 'Nizsi admin musi mit jasne uvedene omezeni hesel dalsich uctu a ownera');
  assertIncludes(ui, 'rakAdminLoadAccountsDirectoryForViewer', 'Prehled spravcu musi nacist bezpecny adresar i pro nizsiho admina');
  ...
  assertIncludes(stylesAdminPolishCss, 'width:min(100%,341px) !important;', 'Zalohy nastaveni musi mit konkretni tabulku zvetsenou o dalsich 5% z 325px na 341px');
...
function assertAdminHandoverGuideContractV1142() {
  ...
  assertIncludes(stylesAdminPolishCss, 'width:min(100%,345px) !important;', 'Zalohy rozpisu musi mit konkretni tabulku zvetsenou o 15% z 300px na 345px');
...
function assertAdminGeneratorSettingsContractV1143() {
  ...
  assertIncludes(ui, 'softKindGlobalBalanceEnabled', 'Pravidla generatoru musi umet zapnout vyrovnani soustruhy/frezky pro celou mekotu');
  assertIncludes(ui, 'adminGeneratorSoftKindGlobalBalanceEnabled', 'Admin formular musi mit volbu pro vyrovnani soustruhy/frezky pres celou mekotu');
  assertIncludes(ui, 'softKindMixedMinimumShifts', 'Pravidla generatoru musi umet nastavit od kolika smen na mekote ma clovek mit oba typy prace');
  assertIncludes(ui, 'adminGeneratorSoftKindMixedMinimumShifts', 'Admin formular musi mit pole pro minimalni pocet smen pro mix soustruh/frezka');
  assertIncludes(ui, 'finalSoftKindBalance', 'Generator musi dorovnat soustruhy/frezky jeste po pozdnich opravach rozpisu');
  ...
  assertIncludes(browserSmokeJs, 'pendingDraftEditorState', 'Browser smoke musi hlidat predani vygenerovaneho navrhu do editoru');
  assertIncludes(ui, 'const month = pendingMonth || savedMonth', 'Editor rozpisu musi uprednostnit cekajici vygenerovany navrh pred online verzi');
  ...
  assertIncludes(ui, "'soft-core-hard-machine'", 'Validace generatoru musi hlidat, ze Synek/Trasak/Strizek nechodi na jine tvrdotni stroje nez TNKS01/TPKW01/TPKW02');
  assertIncludes(ui, "'soft-core-sequence'", 'Validace generatoru musi odmitnout preskok trojice na dalsi stroj pred dokoncenim bloku');
  assertIncludes(ui, 'adminRotationGeneratorFindSoftCoreSequenceIssues', 'Generator musi zaverecne zkontrolovat navaznost bloku trojice proti predchozim mesicum');
  assertIncludes(ui, '!softHardCycle.includes(machine)', 'Generator musi zakazat trojici z mekoty tvrdotni stroje mimo jejich cyklus');
  assertIncludes(ui, "reason === 'soft-core-hard-block'", 'Trojici smi na tvrdotu zapsat jen vyhrazeny blokovy krok generatoru');
  assertIncludes(ui, 'Tento cyklus má přednost před ostatním dorovnáním.', 'Admin pravidla musi vyslovne uvadet prioritu cyklu trojice');
...
function assertAdminFhbCalibrationContractV1166() {
  assertIncludes(read('app.js'), '"admin-fhb-calibration.js"', 'Kalibrace FHB musi byt nactena jako modul aplikace');
  assertIncludes(adminFhbCalibrationJs, "const KEY = 'FHB_CORRECTION_CALIBRATION_SETTINGS'", 'Kalibrace FHB musi mit vlastni settings klic');
  assertIncludes(adminFhbCalibrationJs, 'const MIN_SAMPLES = 3', 'Doporuceni nesmi vzniknout z jednoho mereni');
  assertIncludes(adminFhbCalibrationJs, 'function applyRecommendation', 'Model se musi menit jen pres rucne potvrzene doporuceni');
  assertIncludes(appMenuJs, "action: 'open-correction-settings', label: 'Nastavení korekcí'", 'Administrace musi nabidnout nastaveni korekci');
  assertIncludes(appMenuJs, "adminAction === 'save-fhb-calibration-record'", 'Mereni FHB se musi umet bezpecne ulozit');
  assertIncludes(appMenuJs, "adminAction === 'apply-fhb-calibration'", 'Doporuceni FHB musi vyzadovat samostatne potvrzeni');
  assertIncludes(appMenuJs, "'admin-correction-settings'", 'Nastaveni korekci musi byt chranene admin view');
  assertIncludes(bridge, "category === 'fhb_correction_calibration_settings'", 'Specialni kalibrace musi projit pres zabezpecene ulozeni nastaveni');
  assertIncludes(read('soustruhy.js'), 'window.getFhbCorrectionModel', 'Kalkulacka frezek musi cist az potvrzeny model korekci');
}
...
assertAdminFhbCalibrationContractV1166();
