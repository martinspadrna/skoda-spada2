#!/usr/bin/env node
// RaK 1.2 (1.150) – smoke test přehledu připojení + Dashboard/appearance contract guard.
const fs = require('fs');
...
const releaseMetadataContractV199 = Object.freeze({
  displayVersion: '1.2 (1.150)',
  appLabel: 'RaK 1.2 (1.150)',
  packageVersion: '1.2.150',
  cacheVersion: 'v1.2-1.150',
  realtimeChannel: 'rak-public-live-v1-2-1-126',
  changelogHeader: '## RaK 1.2 (1.150)',
...
function assertReleaseMetadataContractV199() {
  ...
  assertIncludes(exportJs, "displayVersion: '1.2 (1.150)'", 'Release contract v export.js musí držet display verzi 1.105');
  assertIncludes(exportJs, "packageVersion: '1.2.150'", 'Release contract v export.js musí držet package verzi 1.2.114');
...
function assertMemory8x8SquareFitContractV1150() {
  assertIncludes(gamesArcadeJs, '__rakMemory8x8SquareFitGuard', 'Pexeso 8×8 musí mít square-fit guard v1.150');
  ...
  assertIncludes(stylesOverridesCss, 'Pexeso 8×8: čtvercové karty a mobilní fit', 'CSS musí dokumentovat 8×8 square-fit fix');
  assertIncludes(stylesOverridesCss, 'grid-template-rows:repeat(8,minmax(0,1fr))', '8×8 board musí mít i osm řádků, nejen sloupce');
  assertIncludes(stylesOverridesCss, 'width:min(100%,336px,calc(100vw - 28px),calc(100dvh - 280px))', '8×8 board musí být omezený šířkou i výškou viewportu');
  assertIncludes(stylesOverridesCss, 'height:100% !important', '8×8 karty musí vyplnit buňku na výšku');
}

function assertSudokuCompletionAndTimeFormatContractV1144() {

  assertIncludes(gamesArcadeJs, '__rakShipsSmoothPerformanceGuard', 'Lodě musí mít performance guard 1.150');
...
assertMemory8x8SquareFitContractV1150();
