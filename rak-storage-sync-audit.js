// v.1.5 (891) – storage/sync audit closure, smoke report a cleanup guard bez automatického mazání dat.
...
(function setupRakStorageSyncAudit() {
  ...
  function computeRakStorageSyncAuditHealth() {
    ...
    return {
      ok: issues.length === 0,
      mode: 'storage-sync-readonly-closure-v891',
      checkedAt: nowIso(),
  ...
  function getRakStorageManualCleanupGuard() {
    ...
    return {
      ok: true,
      mode: 'manual-cleanup-guard-v891',
      checkedAt: nowIso(),
      version: safeString(window.APP_VERSION || 'unknown'),
      readOnly: true,
      manualOnly: true,
  ...
  function updateRakStorageSyncSmokeReport(partial) {
    ...
    Object.assign(STORAGE_SYNC_SMOKE_REPORT, data, {
      mode: 'storage-sync-smoke-report-v891',
      version: safeString(window.APP_VERSION || 'v.1.5 (891)'),
      checkedAt: nowIso()
    });
  ...
  function getRakStorageSyncSmokeReport() {
    return Object.assign({}, STORAGE_SYNC_SMOKE_REPORT, {
      ...
      mode: safeString(STORAGE_SYNC_SMOKE_REPORT.mode || 'storage-sync-smoke-report-v891'),
  ...
  function getRakStorageSyncClosureHealth() {
    ...
    return {
      ok: issues.length === 0,
      mode: 'storage-sync-audit-closure-v891',
      checkedAt: nowIso(),
      version: safeString(window.APP_VERSION || 'v.1.5 (891)'),
      phase: 'phase G storage/localStorage/offline sync audit',
      phasePercent: 100,
