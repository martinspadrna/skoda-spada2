// v.1.5 (895) – DOM/action registry audit uzavřený, bez přepojení handlerů.
...
(function setupRakDomActionRegistryAudit() {
  ...
  const DOM_ACTION_SMOKE_REPORT = window.__RAK_DOM_ACTION_SMOKE_REPORT__ || (window.__RAK_DOM_ACTION_SMOKE_REPORT__ = {
    ok: null,
    status: 'not-run',
    mode: 'dom-action-smoke-report-v895',
    version: 'v.1.5 (895)',
    checkedAt: null,
  ...
  function updateDomActionSmokeReportFromHealth(health, stage) {
    try {
      ...
      DOM_ACTION_SMOKE_REPORT.mode = 'dom-action-smoke-report-v895';
      DOM_ACTION_SMOKE_REPORT.version = String(window.APP_VERSION || 'v.1.5 (895)');
  ...
  function collectDomActionRegistryHealth() {
    ...
    const health = {
      ...
      targetAttributeRule: 'v895 uzavírá read-only DOM/action registry audit; cílové atributy jsou jen diagnostická mapa pro bezpečný budoucí refactor handlerů.',
  ...
  window.getRakDomActionRegistryClosureHealth = function getRakDomActionRegistryClosureHealth() {
    ...
    return {
      ok: !!(health && health.ok),
      mode: 'dom-action-registry-closure-v895',
      version: String(window.APP_VERSION || 'unknown'),
      checkedAt: new Date().toISOString(),
      phase: 'phase E DOM/action registry audit closure',
      phasePercent: 100,
