// v.1.5 (896) – DOM/action registry audit uzavřený, bez přepojení handlerů.
...
(function setupRakDomActionRegistryAudit() {
  ...
  const DOM_ACTION_SMOKE_REPORT = window.__RAK_DOM_ACTION_SMOKE_REPORT__ || (window.__RAK_DOM_ACTION_SMOKE_REPORT__ = {
    ok: null,
    status: 'not-run',
    mode: 'dom-action-smoke-report-v896',
    version: 'v.1.5 (896)',
    checkedAt: null,
  ...
  function updateDomActionSmokeReportFromHealth(health, stage) {
    try {
      ...
      DOM_ACTION_SMOKE_REPORT.mode = 'dom-action-smoke-report-v896';
      DOM_ACTION_SMOKE_REPORT.version = String(window.APP_VERSION || 'v.1.5 (896)');
  ...
  function collectDomActionRegistryHealth() {
    ...
    const health = {
      ok: issues.length === 0,
      mode: 'dom-action-registry-audit-closure-v896',
      ...
      targetAttributeRule: 'v896 uzavírá read-only DOM/action registry audit; cílové atributy jsou jen diagnostická mapa pro bezpečný budoucí refactor handlerů.',
  ...
  window.getRakDomActionRegistryClosureHealth = function getRakDomActionRegistryClosureHealth() {
    ...
    return {
      ok: !!(health && health.ok),
      mode: 'dom-action-registry-closure-v896',
      version: String(window.APP_VERSION || 'unknown'),
      checkedAt: new Date().toISOString(),
      phase: 'phase E DOM/action registry audit closure',
      phasePercent: 100,
