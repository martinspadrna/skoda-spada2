...
assert.equal(pkg.version, '1.5.99', 'package version must be 1.5.99');
assert.match(app, /RAK_MODULE_CACHE_VERSION\s*=\s*["']1\.5\.99["']/, 'app cache version must be 1.5.99');
assert.match(app, /RAK_DEV_UPDATE_BUILD\s*=\s*["']v1\.5\.99["']/, 'app update build must be v1.5.99');
assert.match(sw, /CACHE_VERSION\s*=\s*["']v1\.5\.99["']/, 'service worker cache must be v1.5.99');
assert.match(sw, /SW_APP_VERSION\s*=\s*["']1\.5\.99["']/, 'service worker app version must be 1.5.99');
assert(sw.includes("'./core.js?v=1.5.99'"), 'warm-start core must use current build');
...
assert(fit.includes('RaK v1.5.97/v1.5.99 – kanonický compact Dashboard owner'), 'v1.5.99 compact owner marker missing');
...
assert(fit.includes('padding-bottom:calc(68px + env(safe-area-inset-bottom)) !important;'), 'compact Home safe-area padding missing');
assert(fit.includes('max-height:calc(100dvh - 76px - env(safe-area-inset-bottom)) !important;'), 'compact shell max-height owner missing');
assert(fit.includes('border-radius:21px !important;'), 'compact shell radius owner missing');
...
assert(fit.includes('max-height:24px !important;') && fit.includes('overflow:hidden !important;'), 'compact metadata clipping owner missing');
assert(fit.includes('padding-left:10px !important;') && fit.includes('padding-right:10px !important;'), 'short-height hero horizontal padding missing');
assert(!fit.includes('min-height:var(--rak-dashboard-fit-card-min) !important;'), 'dead card min-height must not return to fit layer');
assert(!fit.includes('padding:9px 10px !important;'), 'dead compact card padding must not return to fit layer');
assert(!fit.includes('width:32px !important;'), 'dead compact icon size owner must not return to fit layer');
...
assert(String(pkg.scripts.check || '').includes('tools/v1599-smoke.mjs'), 'v1.5.99 smoke must run in npm check');
assert(!String(pkg.scripts.check || '').includes('tools/v1598-smoke.mjs'), 'old v1.5.98 smoke must not remain in active check chain');

console.log('[v1.5.99-smoke] OK dead Dashboard fit owners removed; active compact/device/mobile owners preserved');
