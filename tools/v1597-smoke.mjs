...
assert.equal(pkg.version, '1.5.97', 'package version must be 1.5.97');
assert.match(app, /RAK_MODULE_CACHE_VERSION\s*=\s*["']1\.5\.97["']/, 'app cache version must be 1.5.97');
assert.match(app, /RAK_DEV_UPDATE_BUILD\s*=\s*["']v1\.5\.97["']/, 'app update build must be v1.5.97');
assert.match(sw, /CACHE_VERSION\s*=\s*["']v1\.5\.97["']/, 'service worker cache must be v1.5.97');
assert.match(sw, /SW_APP_VERSION\s*=\s*["']1\.5\.97["']/, 'service worker app version must be 1.5.97');
assert(sw.includes("'./core.js?v=1.5.97'"), 'warm-start core must use current build');
...
assert(fit.includes('min-height:74px !important;') && fit.includes('padding:8px 9px !important;'), 'short-height compact card owner drifted');
assert(fit.includes('min-width:min(206px, 72vw) !important;'), 'canonical absence pill width drifted');
assert(fit.includes('padding:7px clamp(15px, 4.2vw, 30px) 8px !important;'), 'canonical absence pill padding drifted');
...
assert(String(pkg.scripts.check || '').includes('tools/v1597-smoke.mjs'), 'v1.5.97 smoke must run in npm check');
assert(!String(pkg.scripts.check || '').includes('tools/v1596-smoke.mjs'), 'old v1.5.96 smoke must not remain in active check chain');

console.log('[v1.5.97-smoke] OK consolidated Dashboard compact/hero owners + v1.5.96 visual behavior preserved');
